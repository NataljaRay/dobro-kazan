...
// + онлайн-кластер (зелёный) и кластер без координат (красный)
...
async function main() {
    ...
    function createCustomMarker(markerData) {
        ...
        if (markerData.isNoCoords) {
            markerEl.classList.add('map-point--no-coords');
        }

        if (markerData.isOnline) {
            markerEl.classList.add('map-point--online');
        }

        // позиционирование виртуальных кластеров
        if (markerData.isOnline) {
            markerEl.style.position = 'absolute';
            markerEl.style.transform = 'translate(10px, 10px)';   // зелёный сверху
        }

        if (markerData.isNoCoords) {
            markerEl.style.position = 'absolute';
            markerEl.style.transform = 'translate(10px, 60px)';  // красный ниже зелёного
        }
    ...
    function buildClusters(markers, zoom) {
        ...
        markers.forEach(src => {
            ...
            // проверяем онлайн по event.address, а не по src.address
            const isOnlineEvent =
                Array.isArray(src.events) &&
                src.events.some(ev => {
                    const a = (ev.address || '').trim().toLowerCase();
                    return a === 'онлайн' || a === 'online';
                });

            // ---------- 1) Онлайн кластер (зелёный) ----------
            if (isOnlineEvent) {
                if (!onlineCluster) {
                    onlineCluster = {
                        id: 'online',
                        coords: [],          // виртуальный, рисуем в оверлее
                        events: [],
                        isOnline: true,
                        isNoCoords: false,
                    };
                ...
                onlineCluster.events = onlineCluster.events.concat(src.events);
                return;
            }

            // ---------- 2) Кластер без координат (красный) ----------
            if (!hasCoords) {
                if (!noCoordsCluster) {
                    noCoordsCluster = {
                        id: 'no-coords',
